...
annotate_rep <- merge_repeat %>% 
  inner_join(repeats_db, by="repeat_gene")
...
the method used to detect outlier based on allelic length (regardless of motifs). The higher the score is,
...
for (i in 1:nrow(annotate_rep)){
  ...
  if(grepl(row$pathogenic_motifs, row$MOTIFS)){
...
sig_rep <- annotate_rep %>% 
  filter(max_copy > pathogenic_low) %>% 
...
if(nrow(disease_sig_rep) != 0){
  for (i in 1:nrow(disease_sig_rep)){
    ...
    repeat_id <- disease_sig_rep[i, ]$Gene
  ...
  to_vis_trvz <- to_show_tab %>% 
    filter(`Pathogenic motif high?` == "Yes",
           toshow == 1)
